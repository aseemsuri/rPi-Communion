...
echo "=========================================="
echo ""
if [ ! -f python/sensor_config.json ]; then
    echo "!!  REQUIRED NEXT STEP — this box has no active config yet."
    echo "    sensor_config.json is per-box state and is NOT in git."
    echo "    Pick a profile, then set this box's node_id:"
    echo ""
    echo "      cp configs/rods_proximity.json python/sensor_config.json   # or altar_touch.json"
